...
void ChessEngineBestMoveFinder::SaveMovesPathToFile(const ChessScoredMovesPath& ChessScoredMovesPathToSave, ofstream& FileToWriteText)
{
	try
	{
		for (const ChessMove& ChosenChessMove : ChessScoredMovesPathToSave.MovesPath)
...
void ChessEngineBestMoveFinder::ReadChosenMovesPathsFromFile(vector<vector<string>>& LastChosenMovesPathsToMate)
{
	try
	{
		...
		LastChosenMovesPathsToMate.push_back(vector<string>());
...
void ChessEngineBestMoveFinder::SaveChosenMovesPathsToFile(const char* TypeStr) const
{
	try
	{
		...
		if (TypeStr == "CAPTURE")
		{
			if (ChessEngineResultsStatisticsLocalReference.TheChosenMovesPathForAdvantageOf[White].MovesPath.size() > 0)
...
void ChessEngineBestMoveFinder::PrintLookingForEqualMoveFromChosenMovesPaths(const vector<vector<string>>& LastChosenMovesPathsToMate, const string& LastEngineMoveFrom, const string& LastEngineMoveTo, const string& LastOpponentMoveFrom, const string& LastOpponentMoveTo) const
{
	try
	{
		if (LastChosenMovesPathsToMate.size() < ChessEngineConfigurationFileReaderWriterObject.PrintOnlyIfChosenMovesPathsToMateIsSmallerThanThisNumber)
		{
			if (ChessEngineConfigurationFileReaderWriterObject.PrintLookingForEqualMoveFromChosenMovesPaths == true)
				for (const auto& LastChosenMovesPath : LastChosenMovesPathsToMate)
					if (LastChosenMovesPath.size() > 0)
					{
						...
						{
							LoggersManagerObject.Log(STREAM("EQUALE TO MOVE = " << LastChosenMovesPath[2] << " -> " << LastChosenMovesPath[3]));
			...
			LoggersManagerObject.Log(STREAM("Chosen moves paths to mate to big to be printed!" << EndLineChar));
	}
			...
			if (ReadFirstLineFromFile(".." OS_DIR_SEP "workdir" OS_DIR_SEP "chosen_moves_paths_type.txt") == TypeStr)
			{
				...
				if (LastOpponentMove != "MATE")
				{
					...
					if (TheHighestScoredLastChosenMovesPathToMate.empty() == false)
						if (ChessEngineConfigurationFileReaderWriterObject.PrintToMovesFile == true)
						{
							RewriteTextToFile(".." OS_DIR_SEP "workdir" OS_DIR_SEP "last_actual_move.txt", string("T0[" + TheHighestScoredLastChosenMovesPathToMate[4] + " " + TheHighestScoredLastChosenMovesPathToMate[5] + "]").c_str());
							RewriteTextToFile(".." OS_DIR_SEP "workdir" OS_DIR_SEP "last_engine_move.txt", string("T0[" + TheHighestScoredLastChosenMovesPathToMate[4] + " " + TheHighestScoredLastChosenMovesPathToMate[5] + "]").c_str());
					...
					if (TheHighestScoredLastChosenMovesPathToMate.empty() == true && NewLastChosenMovesPathsToMate.size() > 0)
						if (ChessEngineConfigurationFileReaderWriterObject.PrintToMovesFile == true)
						{
							RewriteTextToFile(".." OS_DIR_SEP "workdir" OS_DIR_SEP "last_actual_move.txt", string("M0[" + NewLastChosenMovesPathsToMate[0][4] + " " + NewLastChosenMovesPathsToMate[0][5] + "]").c_str());
							RewriteTextToFile(".." OS_DIR_SEP "workdir" OS_DIR_SEP "last_engine_move.txt", string("M0[" + NewLastChosenMovesPathsToMate[0][4] + " " + NewLastChosenMovesPathsToMate[0][5] + "]").c_str());
...
void ChessEngineBestMoveFinder::FindRandomFirstPossibleMove(ColorType Color)
{
	try
	{
		if (ChessEngineConfigurationFileReaderWriterObject.PrintToMovesFile == true)
		{
			...
			FileUtils::RewriteTextToFile(".." OS_DIR_SEP "workdir" OS_DIR_SEP "last_actual_move.txt", string_utils::trim_whitespace_surrounding(ChessEngineConfigurationFileReaderWriterObject.FirstPossibleMove).c_str());
			FileUtils::RewriteTextToFile(".." OS_DIR_SEP "workdir" OS_DIR_SEP "last_engine_move.txt", string_utils::trim_whitespace_surrounding(ChessEngineConfigurationFileReaderWriterObject.FirstPossibleMove).c_str());
		}
...
void ChessEngineBestMoveFinder::ChooseBestMove()
{
	try
	{
		...
		{
			if (ChessEngineResultsStatisticsLocalReference.AllFoundMovesPathsToStalemate.size() == 1 && ChessEngineResultsStatisticsLocalReference.AllFoundMovesPathsToStalemate[0].MovesPath.size() == 0)
			{
				WriteGameResultToFiles("Chessboard stalemate on start", "STALEMATE");
				return;
			}

			if (ChessEngineResultsStatisticsLocalReference.AllFoundMovesPathsToMate[White].size() == 1 && ChessEngineResultsStatisticsLocalReference.AllFoundMovesPathsToMate[White][0].MovesPath.size() == 0)
			{
			...
			for (ColorType Color = White; Color <= ColorEnd; Color++)
				if (ChessEngineResultsStatisticsLocalReference.AllFoundMovesPathsToMate[1 - Color].empty() == true)
				{
					...
					{
						if (ChessEngineResultsStatisticsLocalReference.TheHighestScoredFoundCaptureMovesPathForAdvantageOf[Color].MovesPath.size() > 0)
				...
				{
					if (ChessEngineResultsStatisticsLocalReference.TheHighestScoredFoundMateMovesPathForAdvantageOf[Color].MovesPath.size() > 0)
			...
			if (ChessEngineConfigurationFileReaderWriterObject.PrintToMovesFile == true)
			{
				vector<ChessMove> MovesPath;
				MovesPath.push_back(ChessEngineResultsStatisticsLocalReference.TheChosenMovesPathForAdvantageOf[White].MovesPath[0]);
				FileUtils::RewriteTextToFile(".." OS_DIR_SEP "workdir" OS_DIR_SEP "last_actual_move.txt", string_utils::trim_whitespace_surrounding(GetChessMovesPathAsString(MovesPath)).c_str());
				FileUtils::RewriteTextToFile(".." OS_DIR_SEP "workdir" OS_DIR_SEP "last_engine_move.txt", string_utils::trim_whitespace_surrounding(GetChessMovesPathAsString(MovesPath)).c_str());
			}
		}
	}
