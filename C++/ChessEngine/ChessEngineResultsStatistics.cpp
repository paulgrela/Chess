...
void ChessEngineResultsStatistics::SumStatisticsFromTwoChessEngineResultsStatisticsObjects(ChessEngineResultsStatistics& ChessEngineResultsStatisticsObjectMain, const ChessEngineResultsStatistics& ChessEngineResultsStatisticsObjectToAdd)
{
	try
	{
        ...
        if ((ChessEngineResultsStatisticsObjectToAdd.TheHighestScoredFoundMateMovesPathForAdvantageOf[White].Score < ChessEngineResultsStatisticsObjectMain.TheHighestScoredFoundMateMovesPathForAdvantageOf[White].Score) || (ChessEngineResultsStatisticsObjectMain.TheHighestScoredFoundMateMovesPathForAdvantageOf[White].MovesPathSize == 0 && ChessEngineResultsStatisticsObjectToAdd.TheHighestScoredFoundMateMovesPathForAdvantageOf[White].MovesPathSize > 0))
        ...
        if ((ChessEngineResultsStatisticsObjectToAdd.TheHighestScoredFoundMateMovesPathForAdvantageOf[Black].Score > ChessEngineResultsStatisticsObjectMain.TheHighestScoredFoundMateMovesPathForAdvantageOf[Black].Score) || (ChessEngineResultsStatisticsObjectMain.TheHighestScoredFoundMateMovesPathForAdvantageOf[Black].MovesPathSize == 0 && ChessEngineResultsStatisticsObjectToAdd.TheHighestScoredFoundMateMovesPathForAdvantageOf[Black].MovesPathSize > 0))
...
void ChessEngineResultsStatistics::ClearAllStatistics()
{
	try
	{
		...
		for (ColorType Color = White; Color <= Black; Color++)
		{
			...
			AllFoundMovesPathsToMate[Color].reserve(256 * 1024);
			TheHighestScoredFoundCaptureMovesPathForAdvantageOf[Color] = { 0, 0, {}, 0 };
			TheHighestScoredFoundMateMovesPathForAdvantageOf[Color] = { 0, 0, {}, 0 };
			TheChosenMovesPathForAdvantageOf[Color] = { 0, 0, {}, 0 };
		}
        TheHighestScoredFoundStalemateMovesPath = { 0, 0, {}, 0 };
		AllFoundMovesPathsToStalemate.clear();
	}
	CATCH("clearing all statistics")
}

const uint64_t ChessEngineResultsStatistics::GetNumberOfAllMovesTotal() const
{
...
void ChessEngineResultsStatistics::PrintGeneralStatistics() const
{
	try
	{		
		LoggersManagerObject.Log(STREAM("Number of all moves: " << GetNumberOfAllMovesTotal() << endl));

		LoggersManagerObject.Log(STREAM("Number of all recursive moves: " << NumberOfAllRecursiveMoves << endl));

		LoggersManagerObject.Log(STREAM("Number of all white pieces moves: " << NumberOfAllPiecesMovesByColor[White]));
		LoggersManagerObject.Log(STREAM("Number of all black pieces moves: " << NumberOfAllPiecesMovesByColor[Black] << endl));
		...
		LoggersManagerObject.Log(STREAM("Number of all moves when king black is checked: " << NumberOfAllMovesWhenCheckedKingColor[Black] << endl));
		...
		LoggersManagerObject.Log(STREAM("Number of all pieces lost by white and removed by black: " << NumberOfAllOppositePiecesRemovedByMoveOfColor[Black] << endl));
		...
		LoggersManagerObject.Log(STREAM("Number of all moves black cancelled because of discovered possible check of king black: " << NumberOfAllMovesCancelledBecauseOfDiscoveredPossibleCheckOfKingColor[Black] << endl));
	}
...
void ChessEngineResultsStatistics::PrintEveryMoveOnChessBoard(const ChessScoredMovesPath& OnePathToMate)
{
	try
	{
        ...
        for (uint64_t ChessMoveIndex = 0; ChessMoveIndex < OnePathToMate.MovesPathSize; ChessMoveIndex++)
        {
            ChessMove ChessMoveObject = OnePathToMate.MovesPath[ChessMoveIndex];
            ChessEngineAllPossibleMovesGeneratorLocalReference.ChessBoard[ChessMoveObject.NextPosX][ChessMoveObject.NextPosY] = ChessEngineAllPossibleMovesGeneratorLocalReference.ChessBoard[ChessMoveObject.PrevPosX][ChessMoveObject.PrevPosY];
            ChessEngineAllPossibleMovesGeneratorLocalReference.ChessBoard[ChessMoveObject.PrevPosX][ChessMoveObject.PrevPosY] = SpaceNum;
	...
	try
	{
		for (auto& OnePathMovesToMate : AllFoundMovesPathsToMate[Color])
		{
			if (ChessEngineConfigurationFileReaderWriterObject.PrintEveryMoveToMateOnChessBoardWhenPrintingStatistics == true)
			{
				...
				PrintEveryMoveOnChessBoard(OnePathMovesToMate);
			}

			LoggersManagerObject.LogWithoutLineInfo(STREAM("PATH TO MATE " << ColorName[Color] << ": " << GetChessMovesPathAsString(OnePathMovesToMate)));

			if (ChessEngineConfigurationFileReaderWriterObject.PrintEveryMoveToMateOnChessBoardWhenPrintingStatistics == true)
				LoggersManagerObject.LogWithoutLineInfo(STREAM(endl));
...
void ChessEngineResultsStatistics::PrintMovesPath(const char* TaskStr, const PosType Color1, const PosType Color2, const ChessScoredMovesPath& ChessScoredMovesPathObject, const bool PrintTheHighestScoredMovesPathWhenPrintingStatistics, const bool PrintTheHighestScoredMovesPathOnChessBoardWhenPrintingStatistics)
{
	try
	{
		...
		if (PrintTheHighestScoredMovesPathOnChessBoardWhenPrintingStatistics == true)
			PrintEveryMoveOnChessBoard(ChessScoredMovesPathObject);

		if (PrintTheHighestScoredMovesPathWhenPrintingStatistics == true)
		{
			LoggersManagerObject.LogWithoutLineInfo(STREAM("PATH TO " << TaskStr << " " << ColorName[Color2] << "s : " << GetChessMovesPathAsString(ChessScoredMovesPathObject)));
			LoggersManagerObject.Log(STREAM(""));
		}
	}
	...
	try
	{
		...
		if (ChessEngineConfigurationFileReaderWriterObject.PrintEveryPathToMateWhenPrintingStatistics == true)
			for (const auto& OnePathMovesToStalemate : AllFoundMovesPathsToStalemate)
				LoggersManagerObject.Log(STREAM("PATH TO MATE STALEMATE: " << GetChessMovesPathAsString(OnePathMovesToStalemate) << endl));
	}
...
void ChessEngineResultsStatistics::SaveTestStatisticsToFile(const ChessEngineTest& ChessTestObjectParameter, const uint64_t TestId) const
{
	try
	{
		if (ChessEngineConfigurationFileReaderWriterObject.WriteTestsStatisticsToFile == true && ChessTestObjectParameter.WriteTestStatisticsToFile == true)
		{
...
void ChessEngineResultsStatistics::CompareTestStatisticsSingleData(const int64_t DataFromFile, const int64_t DataFromTest, const char* ErrorText, bool& TestOK)
{
	try
	{
