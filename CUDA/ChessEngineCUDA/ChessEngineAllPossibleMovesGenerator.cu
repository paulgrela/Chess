...
vector<ChessMove> ChessEngineAllPossibleMovesGenerator::GetChessMovesPathsVector(const LengthType DeepLevelPathLength) const noexcept
{
...
inline void ChessEngineAllPossibleMovesGenerator::SaveTheBestMovesPath(const LengthType DeepLevelPathLength, ChessScoredMovesPath& TheHighestScoredFoundMovesPathForAdvantageOfWhite, ChessScoredMovesPath& TheHighestScoredFoundMovesPathForAdvantageOfBlack) noexcept
{
	try
	{
		auto SumPathScore = SumActuallyInvestigatedMovesPathScore(DeepLevelPathLength);
		PathScoreType ActualMovesPathScore = get<0>(SumPathScore);
		PathScoreType ActualMovesPathTypeSum = get<1>(SumPathScore);
		...
		if (ForMate == false)
		{
			if (Color == White)
			{
				if (ActualMovesPathScore > TheHighestScoredFoundMovesPathForAdvantageOfWhite.Score)
					TheHighestScoredFoundMovesPathForAdvantageOfWhite = ChessScoredMovesPath{ ActualMovesPathScore, ActualMovesPathTypeSum, GetChessMovesPathsVector(DeepLevelPathLength) };
			}
			else
			{
				if (ActualMovesPathScore < TheHighestScoredFoundMovesPathForAdvantageOfBlack.Score)
					TheHighestScoredFoundMovesPathForAdvantageOfBlack = ChessScoredMovesPath{ ActualMovesPathScore, ActualMovesPathTypeSum, GetChessMovesPathsVector(DeepLevelPathLength) };
			}
		}
		else
		{
			if (Color == White)
			{
				if (ActualMovesPathScore < TheHighestScoredFoundMovesPathForAdvantageOfWhite.Score || TheHighestScoredFoundMovesPathForAdvantageOfWhite.MovesPath.empty() == true)
					TheHighestScoredFoundMovesPathForAdvantageOfWhite = ChessScoredMovesPath{ ActualMovesPathScore, ActualMovesPathTypeSum, GetChessMovesPathsVector(DeepLevelPathLength) };
			}
			else
			{
				if (ActualMovesPathScore > TheHighestScoredFoundMovesPathForAdvantageOfBlack.Score || TheHighestScoredFoundMovesPathForAdvantageOfBlack.MovesPath.empty() == true)
					TheHighestScoredFoundMovesPathForAdvantageOfBlack = ChessScoredMovesPath{ ActualMovesPathScore, ActualMovesPathTypeSum, GetChessMovesPathsVector(DeepLevelPathLength) };
			}
...
inline void ChessEngineAllPossibleMovesGenerator::SaveChessBoardStateForCUDAThreadsExecution() noexcept
{
	try
	{
		ChessEngineThreadDataCUDA ChessEngineThreadStartDataCUDAObject;

		ChessEngineThreadStartDataCUDAObject.DeepLevel = static_cast<LengthTypeCUDA>(DeepLevel + 1);
		ChessEngineThreadStartDataCUDAObject.IsAnyPawnPromoted = IsAnyPawnPromoted;

		memcpy(ChessEngineThreadStartDataCUDAObject.ChessBoard, ChessBoard, sizeof(PieceNumType) * MaxChessSizeX * MaxChessSizeY);

		for (PieceNumType PieceNum = 0; PieceNum < NumberOfPieces; PieceNum++)
		{
			ChessEngineThreadStartDataCUDAObject.Pieces[PieceNum].ActualPosX = static_cast<PosTypeCUDA>(Pieces[PieceNum].ActualPosX);
			ChessEngineThreadStartDataCUDAObject.Pieces[PieceNum].ActualPosY = static_cast<PosTypeCUDA>(Pieces[PieceNum].ActualPosY);
			ChessEngineThreadStartDataCUDAObject.Pieces[PieceNum].Alive = Pieces[PieceNum].Alive;
			ChessEngineThreadStartDataCUDAObject.Pieces[PieceNum].Promoted = Pieces[PieceNum].Promoted;
		}

		for (LengthType ChessMove = 0; ChessMove <= DeepLevel; ChessMove++)
		{
			ChessEngineThreadStartDataCUDAObject.ActuallyInvestigatedMovesPath[ChessMove].Score = static_cast<PathScoreTypeCUDA>(ActuallyInvestigatedMovesPath[ChessMove].Score);
			ChessEngineThreadStartDataCUDAObject.ActuallyInvestigatedMovesPath[ChessMove].Type = static_cast<PathScoreTypeCUDA>(ActuallyInvestigatedMovesPath[ChessMove].Type);
			ChessEngineThreadStartDataCUDAObject.ActuallyInvestigatedMovesPath[ChessMove].PieceNum = ActuallyInvestigatedMovesPath[ChessMove].PieceNum;
			ChessEngineThreadStartDataCUDAObject.ActuallyInvestigatedMovesPath[ChessMove].PrevPosX = static_cast<PosTypeCUDA>(ActuallyInvestigatedMovesPath[ChessMove].PrevPosX);
			ChessEngineThreadStartDataCUDAObject.ActuallyInvestigatedMovesPath[ChessMove].PrevPosY = static_cast<PosTypeCUDA>(ActuallyInvestigatedMovesPath[ChessMove].PrevPosY);
			ChessEngineThreadStartDataCUDAObject.ActuallyInvestigatedMovesPath[ChessMove].NextPosX = static_cast<PosTypeCUDA>(ActuallyInvestigatedMovesPath[ChessMove].NextPosX);
			ChessEngineThreadStartDataCUDAObject.ActuallyInvestigatedMovesPath[ChessMove].NextPosY = static_cast<PosTypeCUDA>(ActuallyInvestigatedMovesPath[ChessMove].NextPosY);
		}

		lock_guard<mutex> LockGuardObject{ ChessEngineThreadsData::ChessEngineThreadsDataArrayMutex };
		ChessEngineThreadsData::ChessEngineThreadsDataArrayPointer[ChessEngineThreadsData::ChessEngineThreadsDataArraySize] = ChessEngineThreadStartDataCUDAObject;
		if (ChessEngineThreadsData::ChessEngineThreadsDataArraySize + 1 < MaxNumberOfChessBoardsForFastExecution)
			ChessEngineThreadsData::ChessEngineThreadsDataArraySize++;
	}
	...
	{	
		...
		if (ChessEngineConfigurationFileReaderWriterObject.PrintWhenRemovingPiece == true)
		{
			if (IsOppositePieceNum<ColorOpposite>(OldNewPieceNum) == true)
			{
				...
				LoggersManagerObject.Log(STREAM("VERIFY REMOVE = ( " << GetChessMovesPathAsString(GetChessMovesPathsVector(DeepLevel)) << "-> " << ChessEngineResultsStatisticsObject.GetNumberOfAllMovesTotal() << " " << int(DeepLevel) << " ) " << string(100, SpaceChar)));
		...
		if (ChessEngineConfigurationFileReaderWriterObject.PrintActualMove == true)
			...
			{
				LoggersManagerObject.Log(STREAM(""));
				LoggersManagerObject.Log(STREAM("MOVE " << ColorName[Color] << " = ( " << GetChessMovesPathAsString(GetChessMovesPathsVector(DeepLevel)) << "-> " << ChessEngineResultsStatisticsObject.GetNumberOfAllMovesTotal() << " " << int(DeepLevel) << " ) " << string(100, SpaceChar)));
		...
		if (DeepLevel < ChessEngineConfigurationFileReaderWriterObject.MaxDeepLevel - 1)
		{
			...
			if (DeepLevel + 2 == ChessEngineConfigurationFileReaderWriterObject.MaxDeepLevel)
				SaveChessBoardStateForCUDAThreadsExecution();
			else
			{
				DeepLevel++;
...
inline bool ChessEngineAllPossibleMovesGenerator::VerifyCheckOfKing()	noexcept
{
	...
	try
	{
		...
		{
			LoggersManagerObject.Log(STREAM(""));
			LoggersManagerObject.Log(STREAM("Inter Move " << ColorName[Color] << " S1 = ( " << GetChessMovesPathAsString(GetChessMovesPathsVector(DeepLevel)) << "-> " << ChessEngineResultsStatisticsObject.GetNumberOfAllMovesTotal() << " " << int(DeepLevel) << " ) " << string(100, SpaceChar)));
		}
		#endif
	}
...
inline void ChessEngineAllPossibleMovesGenerator::UpdateDataForIsCheckAndPossibleMove(const bool IsCheck, const bool PossibleMove) noexcept
{
	try
	{
		...
		{
			LoggersManagerObject.Log(STREAM(""));
			LoggersManagerObject.Log(STREAM("Inter Move " << ColorName[Color] << " S2 = ( " << GetChessMovesPathAsString(GetChessMovesPathsVector(DeepLevel)) << "-> " << ChessEngineResultsStatisticsObject.GetNumberOfAllMovesTotal() << " " << int(DeepLevel) << " ) " << string(100, SpaceChar)));
		}
		...
		if (PossibleMove == false && ChessEngineConfigurationFileReaderWriterObject.PrintPossibleMoveFalseState == true)
		{
			...
			ChessEngineChessboardPrinterObject.LogPrintActualChessBoardStateToStreams(&LoggersManager::LogWithoutLineInfoOnlyToFiles, 0);
			LoggersManagerObject.Log(STREAM(""));
			LoggersManagerObject.Log(STREAM("Poss Move " << ColorName[Color] << " False = ( " << GetChessMovesPathAsString(GetChessMovesPathsVector(DeepLevel)) << "-> " << ChessEngineResultsStatisticsObject.GetNumberOfAllMovesTotal() << " " << int(DeepLevel) << " ) " << string(100, SpaceChar)));
		...
		if (IsCheck == true && PossibleMove == false)
		{
			...
			auto SumPathScore = SumActuallyInvestigatedMovesPathScore(DeepLevel);
			PathScoreType ActualMovesPathScore = get<0>(SumPathScore);
			PathScoreType ActualMovesPathTypeSum = get<1>(SumPathScore);

			ChessEngineResultsStatisticsObject.AllFoundMovesPathsToMate[Color].emplace_back(ChessScoredMovesPath{ ActualMovesPathScore, ActualMovesPathTypeSum, GetChessMovesPathsVector(DeepLevel) });
			...
			if (ChessEngineConfigurationFileReaderWriterObject.PrintActualStateWhenMateFound == true)
			{
				...
				ChessEngineChessboardPrinterObject.LogPrintActualChessBoardStateToStreams(&LoggersManager::LogWithoutLineInfoOnlyToFiles, 0);
				LoggersManagerObject.Log(STREAM("PATH TO MATE: " << GetChessMovesPathAsString(GetChessMovesPathsVector(DeepLevel)) << "-> " << ChessEngineResultsStatisticsObject.GetNumberOfAllMovesTotal() << " " << int(DeepLevel)));
				ChessEngineDataInitializerObject.PrintActualStateOfPieces();
			}
		...
		{ 
			...
			ChessEngineResultsStatisticsObject.AllFoundMovesPathsToStalemate.emplace_back(ChessScoredMovesPath{ ActualMovesPathScore, ActualMovesPathTypeSum, GetChessMovesPathsVector(DeepLevel) });
		}
	}
	CATCH("updating data for ischeck and possiblemove");
}
