...
public:
	[[nodiscard]] std::uint64_t GetNumberOfAllMovesTotal() const;
public:
...
private:
	...
	void PrintEveryMoveOnChessBoard(const std::vector<ChessMove>& OnePathToMate);
private:
	void CompareTestStatisticsSingleData(const int64_t DataFromFile, const std::int64_t DataFromTest, const char* ErrorText, bool& TestOK) const;
public:
	void CompareTestStatistics(const ChessEngineTest& ChessTestObject) const;
	void SaveTestStatisticsToFile(const ChessEngineTest& ChessTestObject, const std::uint64_t TestId) const;
public:
