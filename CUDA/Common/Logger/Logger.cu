...
LoggersManager LoggersManagerObject;

std::mutex Logger::LogMessageCoutMutexObject;
std::mutex LoggersManager::CreateNewLoggerForThreadMutexObject;
...
void Logger::CreateDirectories()
{
	try
	{
		...
		{
			#ifdef WINDOWS_PLATFORM
			_mkdir(string(string(LogDirectory) + string("logs")).c_str());
			_mkdir(string(string(LogDirectory) + string("logs") + OS_DIR_SEP + MainDirectoryName).c_str());
			_mkdir(string(string(LogDirectory) + string("logs") + OS_DIR_SEP + MainDirectoryName + OS_DIR_SEP + TaskName).c_str());
			_mkdir(string(string(LogDirectory) + string("logs") + OS_DIR_SEP + MainDirectoryName + OS_DIR_SEP + TaskName + OS_DIR_SEP + LoggerName).c_str());
			...
			#endif
			#ifdef UNIX_PLATFORM
			experimental::filesystem::create_directory(string(LogDirectory) + string("logs"));
			experimental::filesystem::create_directory(string(LogDirectory) + string("logs") + OS_DIR_SEP + MainDirectoryName);
			experimental::filesystem::create_directory(string(LogDirectory) + string("logs") + OS_DIR_SEP + MainDirectoryName + OS_DIR_SEP + TaskName);
			experimental::filesystem::create_directory(string(LogDirectory) + string("logs") + OS_DIR_SEP + MainDirectoryName + OS_DIR_SEP + TaskName + OS_DIR_SEP + LoggerName);

			for (const string& FileName : LoggersManagerObject.FilesNames)
				experimental::filesystem::create_directory(string(LogDirectory) + string("logs") + OS_DIR_SEP + MainDirectoryName + OS_DIR_SEP + TaskName + OS_DIR_SEP + LoggerName + OS_DIR_SEP + FileName);
			#endif
        }
	}
...
void LoggersManager::InitializeLoggerManagerDataForTask(const string& TaskNameParameter, const std::string& LogDirectoryParameter, const string& ActualDateTimeStrParameter, const bool LogThreadsToSeparatedFilesParameter, const uint64_t FileNumberToIncreaseLineNumberParameter, function<void(const ThreadIdType CurrentThreadId, const uint64_t FileNumber, const string& MessageStr)> DrawMessageFunctionObjectParameter)
{
	try
	{
		...
		ThreadIdType CurrentThreadId = STRING_TO_NUMBER_FROM_STRINGSTREAM(this_thread::get_id());
...
void LoggersManager::LogInColorTerminal(std::ostream& color(std::ostream& s), const std::stringstream& Message)
{
    this->LogOnlyToFiles(Message);
    cout << color << Message.str() << terminal_colors_utils::white << endl;
}
				...
				{
					{
						...
						if (FoundLoggerIterator == LoggersThreadsObjectsPointersMap.end())
							LoggersThreadsObjectsPointersMap[CurrentThreadId] = make_unique<Logger>(LogDirectory.c_str(), ActualDateTimeStr.c_str(), string("THREAD_" + to_string(LoggersThreadsObjectsPointersMap.size() + 1) + "_" + STREAM(CurrentThreadId).str()).c_str(), TaskName.c_str(), CurrentThreadId);
					}
